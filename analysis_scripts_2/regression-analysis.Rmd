...
This notebook is organized by the dependent variable and underlying data of fitted models, not by research question. RQs and paper sections connected to each section of this notebook are as follows:
...
[5. Specify candidate predictor structures for mixed model formulae]
...
# flag for whether to insert a breakpoint before doing the actual analyis
setup_only = FALSE
...
One participant was observed by research staff to be falling asleep throughout their two sessions; we remove any of their responses which are more than two standard deviations away from the mean time of all other participants for the corresponding question, on the assumption that they fell asleep while responding to it.
...
These will be re-used for the positive control models (Section 7). The fixed effect structures and the random effects were both listed in Section 5, so now we build up the random effect structures and combine everything into full formulae with the response variable included.
...
We have previously noted that AIC tends to choose more complex models. This may lead to the hypothesis that the selected models are overfit to spurious correlations that don't exist in the underlying population. We refer skeptical readers to [@barr2013random].
...
if (fit_programming_response_times) {
  ...
  for (f in linear_formulae) {
    ...
    # fit the model
    m = try(lmer(f, data=prog_da, REML=FALSE))
...
if (fit_pos_ctrl_response_times) {
  ...
  for (f in linear_formulae) {
    ...
    # fit the model
    m = try(lmer(f, data=pos_ctrl_da, REML=FALSE))
